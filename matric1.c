...
int matrix_group(int** block, int column, int row){
	int* out = (int*)malloc(column * row * sizeof(int));
	int k = 0;
	for(int i = 0; i < row; i++){
		k = i * column;
...
/*a 是矩阵， dim 保存 a 的行列个数，block表示分块的顺序，block_num分块的数量的平方根*/
double* matrix_split(double **a, int* dim, const int* block, const int block_num){
	int row = block(dim[0], block_num); //每一小块的最大行数，可能小于
	int column  = block(dim[1], block_num);//没一小块的最大列数，可能小于
	...
	for(int i = num - 1; i >= 0; i--){
		...
		//实际矩阵块的行列数
		if(coords[0] + row > dim[0]) 
			row = dim[0] - coords[0];
		if(coords[1] + column > dim[1])
			column = dim[1] - coords[1];
		for(int j = row - 1; j >= 0; --j){
			memcpy(matrixes + tem + j * column, *a + (coords[0] + j) * dim[1] + coords[1], column * sizeof(double));
		}
		matrixes[tem - 2] = row;
